...
public:
	...
	SpectrogramData(CGrid *_grid)
	{
		grid = _grid;
		setInterval(Qt::XAxis, QwtInterval(0,grid->GP.dx*(grid->GP.nx-1)));
		setInterval(Qt::YAxis, QwtInterval(0, grid->GP.dy*(grid->GP.ny-1)));
		setInterval(Qt::ZAxis, QwtInterval(0.0, 1));
...
Plot::Plot(CGrid *grid, QWidget *parent) :
	QwtPlot(parent),
	d_alpha(255)
{
	...
	for (double level = 0.05; level < 1; level += 0.1)
		contourLevels += level;
	d_spectrogram->setContourLevels(contourLevels);

	d_spectrogram->setData(new SpectrogramData(grid));
