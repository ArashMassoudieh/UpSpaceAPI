...
CStringOP::CStringOP(string S)
{
	...
	*/
	...
	{
		if ((i<int(S.size())) && (opertr(S[min(i,(int)S.size()-1)]) < 6))
		{
			operators.push_back(opertr(S[i]));
			op_count++;
		}
		if ((i<int(S.size())) && (S[min(i,(int)S.size()-1)]=='('))
		{
			int prntcnt = 1;
		...
		if ((i<int(S.size())) && (S[min(i,(int)S.size()-1)]=='_'))
		{
			int prntcnt = 0;
		...
		if ((i<int(S.size())) && (isnumber(S[min(i,(int)S.size()-1)]) == true))
		{
			int j=i+1;
		...
		if ((i<int(S.size())) && (S[min(i,(int)S.size()-1)]=='k'))
		{
			int j=i+1;
			while (S[j] != ']') j++;
			{
		...
		if ((i<int(S.size())) && (S[min(i,(int)S.size()-1)]=='c'))
		{
			int j=i+1;
			while (S[j] != ']') j++;
			{
		...
		if ((i<int(S.size())) && (S[min(i,(int)S.size()-1)]=='g'))
		{
			int j=i+1;
			while (S[j] != ']') j++;
			{
		...
		if ((i<int(S.size())) && (S[min(i,(int)S.size()-1)]=='s'))
		{
			int j=i+1;
			while (S[j] != ']') j++;
			{
		...
		if ((i<int(S.size())) && (S[min(i,(int)S.size()-1)]=='e'))
		{
			int j=i+1;
			while (S[j] != ']') j++;
			{
		...
		if ((i<int(S.size())) && (S[min(i,(int)S.size()-1)]=='a'))
		{
			int j=i+1;
			while (S[j] != ']') j++;
			{
...
vector<string> split(const string &s, const vector<char> &del)
{
	...
	for (int i=0; i<int(s.size()); i++)
	{
		for (int jj=0; jj<int(del.size()); jj++)
		if (s[i]==del[jj])
		{
	...
	if (lastdel<int(s.size())) strings.push_back(trim(s.substr(lastdel, s.size()-lastdel)));
	for (int i=0; i<int(strings.size()); i++) strings[i] = trim(strings[i]);
	return strings;
...
vector<string> split(const string &s, char del=',')
{
	...
	if (lastdel<int(s.size())) strings.push_back(trim(s.substr(lastdel, s.size()-lastdel)));  // works w/o trim- Trim can be deleted
	for (int i=0; i<int(strings.size()); i++) strings[i] = trim(strings[i]);					// Trim can be deleted
	return strings;
