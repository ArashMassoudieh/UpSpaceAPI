...
public:
	...
	void create_inv_K_Copula(double dt, double decay=0);
	CVector_arma create_RHS_OU(double dt);
	CVector_arma create_RHS_Copula(double dt, double decay=0);
	void solve_transport_OU(double t_end);
	void solve_transport_Copula(double t_end, double decay=0);
	double time_weight;
	double min_v_x = 0;
	double max_v_x=0;
